def contribution(neuron):
    ...
    for input_ in neuron.inputs_and_variables:
        ...
        if input_ not in contributing_inputs:
            # reduce alpha if possible and add to redistribution value
    ...
    # distribute the redistribution value equally across the Contributing functions
    for function in neuron.contributing_functions:
        if function.value != 0.0:
            neuron.inputs_and_variables[function.input].value['alpha'] += (
                                                                          (weight_redistribution / sum_of_active_inputs)
                                                                          / 2) * function.value
