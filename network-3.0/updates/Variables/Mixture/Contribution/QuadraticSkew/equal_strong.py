def equal_strong(neuron):
    ...
    for input_ in set(neuron.inputs_and_variables) - contributing_inputs:
        ...
        for var in (neuron.inputs_and_variables[input_]):
            ...
            if var.value['alpha'] - (var.value['alpha'] - 1) * neuron.options['variable_rate'] > 1:
                weight_redistribution += ((var.value['alpha'] - 1) * neuron.options['variable_rate'])
            ...
            if var.value['beta'] - (var.value['beta'] - 1) * neuron.options['variable_rate'] > 1:
                weight_redistribution += ((var.value['beta'] - 1) * neuron.options['variable_rate'])
    ...
    for input_ in contributing_inputs:
        ...
        # add new fomposite functions if required
        ...
        for function in activations:
            ...
            if skewable_composite != 'break':
                if skewable_composite.quadratic_skew < 0:
                    redist = neuron.inputs_and_variables[input_][skewable_composite_index].value[
                                 'alpha'] * -skewable_composite.quadratic_skew
                ...
                # otherwise take from beta and give to alpha
                    ...
                    redist = neuron.inputs_and_variables[input_][skewable_composite_index].value[
                                 'beta'] * skewable_composite.quadratic_skew
